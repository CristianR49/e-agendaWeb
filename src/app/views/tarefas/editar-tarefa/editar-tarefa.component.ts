...
import { FormsItemTarefaViewModel } from '../models/forms-itemtarefa.view-model';
import { StatusItemTarefaEnum } from '../models/StatusItemTarefaEnum';
...
export class EditarTarefaComponent implements OnInit {
  ...
  adicionarItem(): void {
    const item: FormsItemTarefaViewModel = {
      titulo: this.tituloItemControl?.value,
      status: StatusItemTarefaEnum.Adicionado,
      concluido: false,
    };
  ...
  removerItem(index: number): void {
    ...
    const valorAtual = grupo?.get('status')?.value as StatusItemTarefaEnum;

    const valorAlternado =
      valorAtual == StatusItemTarefaEnum.Removido
        ? StatusItemTarefaEnum.Inalterado
        : StatusItemTarefaEnum.Removido;
